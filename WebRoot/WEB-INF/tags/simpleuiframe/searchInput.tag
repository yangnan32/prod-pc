...
<%@attribute name="maxLength" rtexprvalue="true"  required="false" description="控件的显示条数" %>
...
<%@attribute name="filterId" rtexprvalue="true" required="false" description="过滤的id集合，不显示的数据" %>
...
<%
    ...
    String textAlignk="";
    String tooltipCls="";
    String clickCls="widgetSearchInput";
...
%>
<div class="widget-selectInput" data-id="<%=id%>" data-url="<%=url%>" data-key="<%=key%>" data-dataName="<%=data%>" data-dataRoot="<%=dataRoot%>" data-height="<%=H%>" data-columns="<%=columns%>" data-maxlength="<%=maxLength%>" data-vals="<%=value%>" data-valtexts="<%=valueText%>" data-filterid="<%=filterId%>" data-tooltipcls="<%=tooltipCls%>" data-textalign="<%=textAlign%>" hotKeyFlag="<%=hotKeyFlag%>">
    <div class="uiframe-mainCon-search" id="${id}Parent" style="width:<%=w%>px;">
        <input type="hidden" class="widgetComboxInput ${group}" maxlength="<%=maxLength%>" id="${id}" value="<%=value%>" <%=nameText%> <%=disableText%> />
        <input type="text" id="${id}Input" <%=emptyTextk%> name="<%=id%><%=uuid%>" value="" <%=disableText%> <%=readonlyText%> class="uiframe-search-text <%=disableInputClass%> <%=cls%>">
    </div>
    <div id="${id}Btn" class="<%=clickCls%> uiframe-search-btn <%=disableInputClass%>"></div>
</div>
<c:choose>
