...
%>
<span id="spanTag<%=id%>">
        <input type="<%=xtype%>" id="<%=id%>" emptyText="${emptyText}" class="widgetDisable widgetClear widgetEmptyText uiframe-textinput <%=emptyTextCls%> ${cls} ${group} <%if(disabledFlag){%> uiframe-textinput-disabled <%}%>" style='<%if(readOnlyFlag){%> cursor: default;<%}%><%=style%>'
