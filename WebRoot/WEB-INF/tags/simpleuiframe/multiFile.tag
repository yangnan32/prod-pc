...
<%@attribute name="fileUlId" rtexprvalue="true"  required="false" description="上传文件显示id" %>
<%@attribute name="maxLength" rtexprvalue="true"  required="false" description="上传文件的最大数" %>
...
%>
<div class="uiframe-file-container widget-multiFile" id="file<%=id%><%=uuid%>" data-id="<%=id%>" data-maxLength="<%=max%>" data-fileUlId="<%=fileUlId%>" data-url="<%=fileUrl%>" data-fileid="<%=fileId%>" data-filevalueid="<%=fileValueId%>" data-fileIdValue="<%=fileIdValue%>" data-fileNameValue="<%=fileNameValue%>" data-autoload="<%=autoLoad%>">
    <input type="hidden" id="${fileId}" <%=idText%> <%=fileIdValueText%> />
    <input type="hidden" id="${fileValueId}" <%=fileNameValueText%> <%=nameText%> />
    <span id="${id}" class="uiframe-module-btn uiframe-cancel-btn uiframe-button-file <%=disableBtnClass%>" style="<%=fontText%>" >添加文件</span>
    <c:choose>
