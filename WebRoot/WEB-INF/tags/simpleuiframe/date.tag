...
<%
    ...
    if(StringUtils.isNotEmpty(disabled) && disabled.equalsIgnoreCase("disabled")){
        ...
        disableBtnClass="uiframeTextDisabled";
    }
    if(value==null||value.equalsIgnoreCase("")){
        value="";
    }
    if(dateType==null||dateType.equalsIgnoreCase("")){
        dateType="dateFmt:'yyyy-MM-dd'";
    }
%>
<input type="text" class="widgetDisable uiframe-input-date ${cls} <%=disableBtnClass%> ${group}" value="<%=value%>" id="${id}" name="${name}" style="${style}" <% if(!(readonly!=null && readonly.equalsIgnoreCase("true"))){%>  onFocus="WdatePicker({<%=dateType%>,onpicking:function(dp){var hiddenDate=document.getElementById('hiddenDate');if(hiddenDate){$('#hiddenDate').val(dp.cal.getNewDateStr());} return;}});$('.uiframe-selectHide').hide();" <%}else {%>  readonly="readonly" <%}%>  <%=disableText%> />
<c:choose>
