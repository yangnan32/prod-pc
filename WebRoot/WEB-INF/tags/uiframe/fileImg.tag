        ...
        var file=function(){
            new AjaxUpload('filebtn${id}', {
                ...
                onComplete: function(file, response) {
                    ...
                    $("#${id}Img").attr("src",basePath +'/download/?ID=' + obj.fileMsg.fileId + '&&DIS=true&ORIGINALNAME=' + encodeURI(encodeURI(obj.fileMsg.fileName)));
                    $("#${id}Img").show();
                }
            });
            ...
            if (<%=fileNameValueFlag%> && <%=fileIdValueFlag%>) {
                ...
                $("#${id}Img").attr("src",basePath +'/download/?ID=' + "<%=fileIdValue%>" + '&&DIS=true&ORIGINALNAME=' + encodeURI(encodeURI("<%=fileNameValue%>")));
                $("#${id}Img").show();
            } else {
                $('#${id}').val("支持输JPG、PNG、GIF、BMP格式的图片文件");
                $('#${id}').addClass("uiframe-emptyTextColor");
...
<div style="height:<%=h%>px;width:<%=w+86%>px;margin-top:10px;overflow:auto;">
    <img id="${id}Img" style="text-align:left;display:none;" src="" />
</div>
