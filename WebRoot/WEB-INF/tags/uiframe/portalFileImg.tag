...
<%@ attribute name="showImgDivId" required="true" description="展示图片div的id"%>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        var addFileList<%=id%> = function(fileId, fileName, fileSize) {
            var delSpan=$('<div class="deleteShareImg"></div>');
            var addSpan=$('<span>' + getDownloadFile<%=id%>(fileId, fileName, fileSize) + '</span>');
        ...
        var getDownloadFile<%=id%> = function(fileId, fileName, fileSize) {
            ...
            if(_downloadEnable){
            %>
            return '<img src="<%=basePath%>/download/?ID=' + fileId + '&&DIS=true&ORIGINALNAME=' + fileName + '" />';
            <%
            } else {
            %>
        ...
        %>
        $("#<%=showImgDivId%>").html("上传文件不存在或已经被移除！");
        <%
        }
        %>
    });
</script>
<div id="<%=id%>" class="shareFileImg"><%=fileName%></div>
<!-- 上传文件控件结束 -->